...
import { fetchAllListingsAdmin, setListingStatus, setListingFeatured } from "@/lib/listings";
...
export default function AdminPage() {
  ...
  const visibleListings = useMemo(() => listings.filter(i => i.status !== "removed"), [listings]);

  const sellers = useMemo(() => {
    const map = new Map<string, { id: string; name: string; location: string; count: number; verified: boolean }>();
    visibleListings.forEach(i => {
      const s = i.seller;
  ...
  const removeListing = async (id: string) => {
    const prev = listings;
    setListings(p => p.map(i => i.id === id ? { ...i, status: "removed" } : i));
    try {
      await setListingStatus(id, "removed");
    } catch (err) {
      setListings(prev);
      setActionError(err instanceof Error ? err.message : "Couldn't remove that listing.");
    }
  };

  const restoreListing = async (id: string) => {
    const prev = listings;
    setListings(p => p.map(i => i.id === id ? { ...i, status: "active" } : i));
    try {
      await setListingStatus(id, "active");
    } catch (err) {
      setListings(prev);
      setActionError(err instanceof Error ? err.message : "Couldn't restore that listing.");
    }
  };
  ...
  const toggleFeature = async (id: string) => {
    ...
    const prev = listings;
    setListings(p => p.map(i => i.id === id ? { ...i, is_featured: nextFeatured } : i));
    try {
      await setListingFeatured(id, nextFeatured);
    } catch (err) {
      setListings(prev);
      setActionError(err instanceof Error ? err.message : "Couldn't update that listing.");
    }
  };

  const stats = [
    { label: "Total Listings", value: visibleListings.length, icon: Package },
    { label: "Active Sellers", value: sellers.length, icon: Users },
    { label: "Featured Items", value: visibleListings.filter(i => i.is_featured).length, icon: Star },
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 page-enter">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <span className="inline-flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/30 text-amber-500 text-[11px] font-bold px-3 py-1.5 rounded-full">
          <span className="w-1.5 h-1.5 rounded-full bg-amber-500" /> No login required — anyone with this URL can moderate
        </span>
      </div>

      {actionError && (
        <div className="mb-6 flex items-center justify-between gap-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3">
          <p className="text-red-700 dark:text-red-300 text-sm font-medium">{actionError}</p>
          <button onClick={() => setActionError(null)} className="text-red-500 hover:text-red-700"><XCircle size={16} /></button>
        </div>
      )}

      {!isSupabaseConfigured && (
        <div className="mb-6 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3">
          <p className="text-red-700 dark:text-red-300 text-sm font-medium">
            Backend isn't configured on this deployment — listing data can't be loaded here.
          </p>
        </div>
      )}
        ...
        <div>
          ...
          <div className="bg-card border border-red-500/20 rounded-xl p-5 mb-6">
            ...
            <div className="space-y-2.5">
              {(Object.entries(
                visibleListings.reduce<Record<string, number>>((acc, i) => { acc[i.category] = (acc[i.category] || 0) + 1; return acc; }, {})
                ...
                <div key={cat} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(count / visibleListings.length) * 100}%` }}
                    />
                  </div>
          ...
          <div className="bg-card border border-red-500/20 rounded-xl p-5">
            ...
            <p className="text-3xl font-black text-red-500">
              {formatMK(visibleListings.reduce((sum, i) => sum + (i.price || 0), 0))}
            </p>
          ...
          <div className="bg-card border border-red-500/20 rounded-xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {loadingListings ? (
                    <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground text-sm">Loading listings…</td></tr>
                    ...
                    <tr key={item.id} className="border-b border-red-500/10 last:border-0 hover:bg-red-500/5">
                      ...
                      <td className="px-4 py-3">
                        {item.status === "removed"
                          ? <span className="inline-flex items-center bg-muted text-muted-foreground text-[10px] font-bold px-2 py-1 rounded-full">Removed</span>
                          : item.status === "sold"
                          ? <span className="inline-flex items-center bg-amber-500/15 text-amber-600 text-[10px] font-bold px-2 py-1 rounded-full">Sold</span>
                          : <span className="inline-flex items-center bg-green-500/15 text-green-600 text-[10px] font-bold px-2 py-1 rounded-full">Active</span>}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {item.status === "removed" ? (
                            <button onClick={() => restoreListing(item.id)} className="p-1.5 rounded-lg hover:bg-green-500/10 text-muted-foreground hover:text-green-600" title="Restore listing">
                              <ArrowLeft size={14} />
                            </button>
                          ) : (
                  ...
                  {!loadingListings && filteredListings.length === 0 && (
                    <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground text-sm">No listings match your search.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
