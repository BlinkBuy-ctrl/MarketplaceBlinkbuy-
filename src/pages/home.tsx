  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 page-enter">
      ...
      <div className="relative overflow-hidden rounded-2xl mb-8">
        ...
        <div className="absolute top-0 right-0 w-96 h-96 bg-red-500/20 rounded-full blur-3xl opacity-40" />
        ...
        <div className="relative px-6 md:px-10 py-10 text-white" data-tour="hero-search">
          ...
          <div className="flex gap-3 flex-wrap">
            <Link
              href="/marketplace"
              className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/15 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all border border-white/20 hover:border-red-500/40 backdrop-blur-sm"
            >
            ...
            <Link
              href="/post-item"
              className="inline-flex items-center gap-2 bg-gradient-to-r from-red-500/80 to-red-600/80 hover:from-red-500 hover:to-red-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all border border-red-400/20"
            >
      ...
      <div className="grid grid-cols-3 gap-3 mb-10">
        {[
          ...
          <div
            key={s.label}
            className="group bg-card border border-red-500/20 hover:border-red-500/40 rounded-xl p-4 text-center transition-all duration-300 hover:shadow-lg hover:shadow-red-500/10 overflow-hidden relative"
          >
            <div className="absolute inset-0 bg-gradient-to-br from-red-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            <div className="relative">
              <s.icon size={18} className="text-red-500 mx-auto mb-1.5 group-hover:scale-110 transition-transform" />
      ...
      {/* TRUST BADGES */}
      <div className="flex flex-wrap items-center justify-center gap-4 mb-10 py-4 px-6 bg-card border border-red-500/15 rounded-xl">
        {[
          ...
          <div key={b.label} className="flex items-center gap-2 text-xs text-muted-foreground font-semibold">
            <b.icon size={13} className="text-red-500" />
            {b.label}
          </div>
        ))}
      </div>
        ...
        <div className="grid grid-cols-5 sm:grid-cols-5 lg:grid-cols-10 gap-2.5">
          {categories.map(cat => (
            <Link
              ...
              className="group flex flex-col items-center gap-2 p-3 bg-card border border-red-500/15 hover:border-red-500/50 rounded-xl transition-all duration-200 hover:shadow-md hover:shadow-red-500/10 cursor-pointer"
            >
              ...
              <span className="text-[10px] font-bold text-center text-muted-foreground group-hover:text-red-500 transition-colors leading-tight line-clamp-1">
                {cat}
              </span>
            </Link>
          ))}
            ...
            <Link key={item.id} href={`/marketplace/${item.id}`}>
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <div className="text-base font-black text-red-500 mb-1">{formatMK(item.price)}</div>
      ...
      <div className="slide-up">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {recent.map((item, i) => (
            <Link key={item.id} href={`/marketplace/${item.id}`}>
              <div
                className="bg-card border border-red-500/20 hover:border-red-500/50 rounded-xl overflow-hidden card-hover cursor-pointer group"
                style={{ animationDelay: `${i * 50}ms` }}
              >
      ...
      <div className="mt-14 mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={s.step} className="bg-card border border-red-500/15 rounded-xl p-5 text-center hover:border-red-500/40 transition-all">
              <div className="text-3xl mb-3">{s.emoji}</div>
              <div className="w-7 h-7 rounded-full bg-gradient-to-br from-red-500 to-red-600 text-white text-xs font-black flex items-center justify-center mx-auto mb-2">{s.step}</div>
      ...
      <div className="mt-6 relative overflow-hidden rounded-2xl">
        ...
        <div className="absolute top-0 right-0 w-80 h-80 bg-red-500/30 rounded-full blur-3xl opacity-20" />
        <div className="relative px-6 md:px-10 py-12 text-center">
          ...
          <Link
            href="/post-item"
            className="inline-flex items-center gap-2.5 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-8 py-3.5 rounded-xl text-sm font-bold transition-all duration-300 shadow-lg hover:shadow-red-500/50 border border-red-400/30"
          >
